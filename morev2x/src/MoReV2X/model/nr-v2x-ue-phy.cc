    ...
    m_waitingNextScPeriod(false)
{
  ...
  // PATCH 2026-05-28: default CBR x-window is wide-open (any scenario).
  // Set via Config::SetDefault("ns3::NrV2XUePhy::CBRxMin"/"CBRxMax",...) to
  // restore 3GPP highway-straight gate (1500, 3500).
  m_CBRxMin = -1e9;
  m_CBRxMax =  1e9;
...
NrV2XUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrV2XUePhy")
    ...
    .AddAttribute ("RSSIthreshold",
     ...
     MakeDoubleChecker<double> ())
    // PATCH 2026-05-28: configurable CBR evaluation window (was hard-coded
    // x in [1500, 3500] for 3GPP highway-straight). Wide-open by default.
    .AddAttribute ("CBRxMin",
                   "Lower bound (m) on UE x-coordinate for CBR sampling",
                   DoubleValue (-1e9),
                   MakeDoubleAccessor (&NrV2XUePhy::m_CBRxMin),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("CBRxMax",
                   "Upper bound (m) on UE x-coordinate for CBR sampling",
                   DoubleValue ( 1e9),
                   MakeDoubleAccessor (&NrV2XUePhy::m_CBRxMax),
                   MakeDoubleChecker<double> ())
  ...
  if (Simulator::Now ().GetSeconds () - m_CBRCheckingInterval > m_CBRCheckingPeriod)
  {
    ...
    for (NodeContainer::Iterator L = GlobalContainer.Begin(); L != GlobalContainer.End(); ++L)
    {
      ...
      if (RxNode->GetId() == m_rnti)
      {
        ...
        // PATCH 2026-05-28: gate is now driven by Attributes CBRxMin/CBRxMax
        // (defaults wide-open). Original 3GPP highway-straight gate
        // (1500..3500) can be re-enabled via Config::SetDefault.
        if (posRX.x >= m_CBRxMin && posRX.x <= m_CBRxMax)
        {
          NrV2XUePhy::UnimoreEvaluateCBR(SF.frameNo, SF.subframeNo);
        }
        else
        {
          NS_LOG_INFO("Rx UE outside of the CBR evaluation window ["
                      << m_CBRxMin << "," << m_CBRxMax << "]");
        }
      }
    }
