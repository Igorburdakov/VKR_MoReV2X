	...
	static TypeId tid = TypeId ("ns3::NrV2XUeMac")
		...
		.AddAttribute ("NumerologyIndex",
                ...
                .AddAttribute ("KeepProbability",
                                        "Probability of keeping the current resource when Cresel reaches zero (slProbResourceKeep / pKeep)",
                                        DoubleValue (0.0),
...
//   m_nsubCHsize (10),
   m_L_SubCh (1),
//   m_BW_RBs (50),
   m_maxPDB(250.0),  // PATCH 2026-05-17: was 110.0; расширено до 250 мс для поддержки RRI=200 (exp06/exp07). 3GPP TS 23.285 допускает PDB до 100 мс для V2X, но MoReV2X использует PDB как абсолютный дедлайн ресурса = T2 ≈ RRI. При RRI=200 мс T2≈200 мс > 110 → assert. Новое значение 250 мс покрывает RRI=200 с запасом.
...
//               PKTsType << itBsr->second.V2XPacketID << ",1," << poolIt->second.m_currentV2XGrant.m_SelectionTrigger << "," << Simulator::Now().GetSeconds() << std::endl;
 ...
 //
      ...
      //       //Pause the program for debugging purposes
